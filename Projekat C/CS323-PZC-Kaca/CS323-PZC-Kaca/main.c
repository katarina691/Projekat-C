...
void setValuesFajl2(Zaposleni* zap, FILE* f, int* prvi) { //Postavljanje vrednosti kada se brise
	if (*prvi != 0) {
		fprintf(f, "\n%s %s %s %s %d",
			zap->ime, zap->prezime, zap->pozicija, zap->JMBG, zap->plata);
	}
...
Zaposleni* uradi(int* brojac, Zaposleni* niz) { //Popunjavanje Array-a, smestanje zaposlenih u niz
	*brojac = 0;
	FILE* fp;
	...
	while (!feof(fp)) {
		...
		Zaposleni z1;
		//citanje zaposlenih iz fajla
		citaj_osobu(&z1, fp);
		//smestanje zaposlenih u niz
		niz[*brojac] = z1;
		(*brojac)++;
	}
	fclose(fp);
...
int main()
{
	...
	while (izbor != 0) { 
		...
		case 3: //Ispis i delete
			...
			for (int i = 0; i < brojac; i++) {
				if (strcmp(niz[i].JMBG, unosJMBG) != 0) {
					setValuesFajl2(&niz[i], fupis2, &prvi);
					//da upisuje bez entera
					prvi = 1;
				}
				else {
					biloga = 1;
